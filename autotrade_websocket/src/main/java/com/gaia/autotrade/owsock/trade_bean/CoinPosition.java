...
public class CoinPosition {
	...
	public final boolean Equal(CoinPosition position) {
		...
		if (m_brokerID.equals(position.m_brokerID) && m_code.equals(position.m_code)
				&& m_exchangeID.equals(position.m_exchangeID) && m_frozenVolume == position.m_frozenVolume
				&& m_investorID.equals(position.m_investorID) && m_volume == position.m_volume) {
			return true;
		}
		return false;
	}

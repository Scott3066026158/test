...
public class CoinOrder{
    ...
    #define ORDERSTATUS_P3  3//部分挂单, 部分撤单
    #define ORDERSTATUS_P4  4//部分挂单, 部分成交
    #define ORDERSTATUS_P5  5//部分挂单, 部分撤单, 部分成交
    #define ORDERSTATUS_P6  6//完全成交
    */
    /**
     * 上面是盛俊杰定义的数据
     */
   ...
   public String GetKeyID()
   {
    ...
    public final boolean Equal(CoinOrder data)
    {
        ...
        if (m_bModify == data.m_bModify
            && m_brokerID.equals(data.m_brokerID)
            && m_cancelTime.equals(data.m_cancelTime)
            ...
            && m_frozenVolume == data.m_frozenVolume
            && m_limitPrice == data.m_limitPrice
            && m_insertDate.equals(data.m_insertDate)
            && m_insertTime.equals(data.m_insertTime)
            && m_investorID.equals(data.m_investorID)
            && m_orderPriceType.equals(data.m_orderPriceType)
            && m_orderRef.equals(data.m_orderRef)
            && m_orderStatus.equals(data.m_orderStatus)
            && m_orderSysID.equals(data.m_orderSysID)
            && m_orderType.equals(data.m_orderType)
            && m_requestID == data.m_requestID
            && m_statusMsg.equals(data.m_statusMsg)
            && m_traderID.equals(data.m_traderID)

...
public class BaseService
{
	...
	public static int ConnectToServer(int proxyType, String ip, int port, String proxyIp, int proxyPort, String proxyUserName, String proxyUserPwd, String proxyDomain, int timeout)
	{
		...
		return Clients.Connect(0, proxyType, ip, (short)port, proxyIp, (short)proxyPort, proxyUserName, proxyUserPwd, proxyDomain, timeout);
	}
	...
	public void OnCallBack(Binary br, int socketID, int localSID, int len)
	{
		try
		{
			...
			if (compressType == COMPRESSTYPE_GZIP)
			{
				...
				message = new CMessage(GetGroupID(), GetServiceID(), functionID, sessionID, requestID, socketID, state, compressType, bodyLength, unzipBody);
			}else
			{
				message = new CMessage(GetGroupID(), GetServiceID(), functionID, sessionID, requestID, socketID, state, compressType, bodyLength, body);
			}
			OnReceive(message);
